...
class Model( object ):
	...
	def __init__( self, name='caffe-model', **kwargs ):
		...
		self.name = name
		self.nodes = {}
		self.policy = { key: value for key, value in **kwargs }
	...
	def compile( self ):
		...
		with open( '{}.prototxt'.format( self.name ), 'w' ) as model:
			model.write( self.to_prototxt() )

		with open( '{}_policy.prototxt'.format( self.name ), 'w' ) as policy:
	...
	def to_prototxt( self ):
		...
		print '\n'.join( self.node.to_prototxt() )

	def to_policy_prototxt( self ):
		"""Convert the parameters to a policy prototxt file."""

		print 'net: {}'.format( self.name + '.prototxt') + \
		      '\n'.join( '{}: {}'.format( key, val ) for key, value in self.policy.items() )
